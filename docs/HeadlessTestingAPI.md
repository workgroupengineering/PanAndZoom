...
16. [ZoomBorder Appium Tests](#zoomborder-appium-tests)
17. [Test Coverage Summary](#test-coverage-summary)
...
## ZoomBorder Appium Tests

The `ZoomBorderAppiumTests` class demonstrates how to test the ZoomBorder control using the Appium-like API. These tests cover all major ZoomBorder functionality using familiar Selenium/Appium patterns.

### Test Categories (73 Tests)

#### Find ZoomBorder Tests

Tests for locating ZoomBorder using various `By` strategies:
...
// By.ClassName (type name)
var element = driver.FindElement(By.ClassName("ZoomBorder"));

// By.Type<T> (generic type)
var element = driver.FindElement(By.Type<ZoomBorder>());

// By.XPath
var element = driver.FindElement(By.XPath("//ZoomBorder"));
```

#### Property Tests

Get and set ZoomBorder properties via the Appium element wrapper:

```csharp
var element = driver.FindElement(By.Type<ZoomBorder>());

// Read properties
double zoomX = element.GetProperty<double>("ZoomX");
double zoomY = element.GetProperty<double>("ZoomY");
bool enablePan = element.GetProperty<bool>("EnablePan");
bool enableZoom = element.GetProperty<bool>("EnableZoom");

// Write properties
element.SetProperty("ZoomSpeed", 2.0);
...
Execute ZoomBorder commands via the element:

```csharp
var zoomBorder = (ZoomBorder)element.Control;
...
Using `TouchAction` for gesture simulation:
...
// Long press
new TouchAction(driver)
    .LongPress(element, durationMs: 500)
    .Perform();

// Swipe
new TouchAction(driver)
    .Press(element)
    .Swipe(SwipeDirection.Left, 100)
    .Release()
    .Perform();
```
...
```csharp
// Pinch zoom in (scale > 1)
MultiTouchAction.Pinch(driver, element, scale: 1.5);
Assert.True(zoomBorder.ZoomX > initialZoom);

// Pinch zoom out (scale < 1)
MultiTouchAction.Pinch(driver, element, scale: 0.5);
Assert.True(zoomBorder.ZoomX < initialZoom);

// Two-finger scroll
MultiTouchAction.Scroll(driver, element, deltaX: 50, deltaY: 30);
```

#### Wait/ExpectedConditions Tests

WebDriverWait-style waits:
...
driver.Wait.Until(d => 
    d.FindElement(By.Type<ZoomBorder>())
     .GetProperty<double>("ZoomX") == 1.0);

// Wait for clickable
var clickable = driver.Wait.Until(
    ExpectedConditions.ElementToBeClickable(By.Type<ZoomBorder>()));
```

#### Child Element Tests

Navigate the visual tree:

```csharp
var zoomBorder = driver.FindElement(By.Type<ZoomBorder>());

// Find child by name
var contentBorder = zoomBorder.FindElement(By.Name("ContentBorder"));

// Find nested text
var textBlock = zoomBorder.FindElement(By.Name("ContentText"));

// Find all children of type
var borders = zoomBorder.FindElements(By.ClassName("Border"));
```
...
#### ZoomBorder Direct Method Tests

Call ZoomBorder methods through the element:
...
zoomBorder.ZoomOut(skipTransitions: true);

// Pan operations
zoomBorder.PanDelta(50, 30, skipTransitions: true);

// Advanced operations
zoomBorder.ZoomToRectangle(new Rect(50, 50, 100, 100), skipTransitions: true);
zoomBorder.CenterOn(new Point(150, 100), skipTransitions: true);
...
Assert.True(zoomBorder.ZoomX <= 4.0);

// Try to zoom below min
for (int i = 0; i < 20; i++)
    zoomBorder.ZoomOut(skipTransitions: true);

Assert.True(zoomBorder.ZoomX >= 0.5);
```

#### Composite Locator Tests

Complex element finding:

```csharp
// Chained search (parent -> child)
var nested = driver.FindElement(By.Chained(
    By.Type<ZoomBorder>(),
...
var windowBitmap = driver.Screenshot();
Assert.NotNull(windowBitmap);
```
...
// Async chained gestures
await new TouchAction(driver)
    .Tap(element)
    .LongPress(element, durationMs: 200)
    .Tap(element)
    .PerformAsync();
```

### Complete ZoomBorder Appium Test Example

```csharp
[AvaloniaFact]
public void ZoomBorder_CompleteAppiumWorkflow()
{
    // Setup
    var zoomBorder = new ZoomBorder
    {
    ...
    zoomBorder.Child = new Border { Background = Brushes.Blue };
    ...
    var byName = driver.FindElement(By.Name("TestZoomBorder"));
    var byAutomation = driver.FindElement(By.AutomationId("zoom_border"));
    var byType = driver.FindElement(By.Type<ZoomBorder>());
    
    Assert.Equal(byName.Control, byAutomation.Control);
    Assert.Equal(byName.Control, byType.Control);

    // Read properties
    Assert.Equal(1.0, byType.GetProperty<double>("ZoomX"));
    Assert.True(byType.GetProperty<bool>("EnablePan"));

    // Perform gestures
    var initialZoom = zoomBorder.ZoomX;
    MultiTouchAction.Pinch(driver, byType, scale: 2.0);
    Assert.True(zoomBorder.ZoomX > initialZoom);

    // Use touch action
    new TouchAction(driver)
        .LongPress(byType, durationMs: 300)
        .Perform();
